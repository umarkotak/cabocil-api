...
func GenCacheUrl(url, preset string, width, height int) string {
	return fmt.Sprintf("%s/ytkidd/api/utils/image/compress?url=%s&w=%v&h=%v&preset=%s", config.Get().AppHost, url, width, height, preset)
}
